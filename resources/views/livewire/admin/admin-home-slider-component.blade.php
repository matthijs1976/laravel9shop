            ...
            <div class="mx-auto">
                <div class="mb-3 float-right">
                    <a href="{{ route('admin.home.slide.add') }}"
                        ...
                        Nieuwe Caroussel Toevoegen
                    </a>
                </div>
                <header class="text-center">
                    <h1 class="text-xl font-bold text-blue-600 dark:text-white sm:text-3xl">Carrousel Afbeeldingen</h1>
                </header>
                <!-- carousel table -->
                        ...
                        <table class="min-w-full bg-white">
                            <thead
                                ...
                                <tr>
                                    ...
                                    <th class="text-left py-3 px-4 uppercase font-semibold text-sm">Afbeelding</th>
                                    <th class="text-left py-3 px-4 uppercase font-semibold text-sm">Top Titel</th>
                                    <th class="text-left py-3 px-4 uppercase font-semibold text-sm">Titel</th>
                                    <th class="text-left py-3 px-4 uppercase font-semibold text-sm">Sub Titel</th>
                                    <th class="text-left py-3 px-4 uppercase font-semibold text-sm">Offer</th>
                            ...
                            <tbody class="bg-white text-blue-600 dark:text-white dark:bg-gray-700">
                                ...
                                @foreach ($slides as $slide)
                                    ...
                                    <td class="text-left py-3 px-4"><img class="object-scale-down h-32 w-80 transition ease-in-out hover:scale-110" src="{{asset('assets/imgs/sliders')}}/{{ $slide->image }}"/></td>
                                    <td class="text-left py-3 px-4">{{ $slide->top_title }}</td>
                                    <td class="text-left py-3 px-4">{{ $slide->title }}</td>
                                    <td class="text-left py-3 px-4">{{ $slide->sub_title }}</td>
                                    <td class="text-left py-3 px-4">{{ $slide->offer }}</td>
                                    <td class="text-left py-3 px-4">{{ $slide->link }}</td>
                                    <td class="text-left py-3 px-4">{{ $slide->status == 1 ? 'Actief':'NietActief'}}</td>
                                    
                                    <td class="flex justify-start gap-8 py-3 px-4"><a
                                            href="{{ route('admin.home.slide.edit', ['slide_id' => $slide->id]) }}"
                                                    ...
                                                    <div x-cloak x-show="modelOpen"
                                                        ...
                                                        <div class="flex items-center justify-between space-x-4">
                                                            <h1 class="text-xl font-medium text-gray-800 ">Bevestig
                                                                Verwijderen {{ $slide->name }} Afbeelding</h1>
